...
trait OverviewPageHelpers extends IntegrationTest with ViewHelpers {
  ...
  object Links {
    ...
    def gainsLinkSummary(taxYear: Int = taxYear): String = s"http://localhost:10005/update-and-submit-income-tax-return/additional-information/$taxYear/gains/summary"

    def gainsLink(taxYear: Int = taxYear): String = s"http://localhost:10005/update-and-submit-income-tax-return/additional-information/$taxYear/gains/gains-gateway"
    ...
    def selfEmploymentLink(taxYear: Int = taxYear): String = s"http://localhost:10901/update-and-submit-income-tax-return/self-employment/$taxYear/task-list"
  ...
  def stubInYearLiabilityCalculation(response: Option[LiabilityCalculationIdModel], returnStatus: Int = OK): StubMapping = {
    stubPost(s"/income-tax/nino/AA123456A/taxYear/$taxYear/tax-calculation\\?crystallise=false", returnStatus, Json.toJson(response).toString())
  }

  def stubLiabilityCalculation(response: Option[LiabilityCalculationIdModel], returnStatus: Int = OK): StubMapping = {
    stubPost(s"/income-tax/nino/AA123456A/taxYear/$taxYear/tax-calculation\\?crystallise=true", returnStatus, Json.toJson(response).toString())
  }

  def stubLiabilityCalculationEOY(response: Option[LiabilityCalculationIdModel], returnStatus: Int = OK): StubMapping = {
    stubPost(s"/income-tax/nino/AA123456A/taxYear/$taxYearEOY/tax-calculation\\?crystallise=true", returnStatus, Json.toJson(response).toString())
  }
