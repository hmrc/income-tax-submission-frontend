...
class OverviewTailoringModelSpec extends UnitTest {
  ...
  "the number of sources match the number of income source fields" in {
    val fromSources: Array[String] = classOf[IncomeSourcesModel].getDeclaredFields.map(field => field.getName).filterNot(_ == "hasDataForEachIncomeSource")
    sources.length shouldBe fromSources.length
  }
