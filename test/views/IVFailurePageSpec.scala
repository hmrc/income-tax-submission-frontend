...
class IVFailurePageSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite with ViewTest{
  ...
  "Rendering the IVFailurePage in English" should {
    "render correctly when the user is an individual" should {
      ...
      lazy val view: Html = iVFailurePage(controllers.routes.SignOutController.signOut(isAgent = false))(fakeRequest, messages, appConfig)
  ...
  "Rendering the IVFailurePage in Welsh" should {
    "render correctly when the user is an individual" should {
      ...
      lazy val view: Html = iVFailurePage(controllers.routes.SignOutController.signOut(isAgent = false))(fakeRequest, welshMessages, appConfig)
      lazy implicit val document: Document = Jsoup.parse(view.body)
