...
import config.ScalamockAppConfig
      ...
      "the call to retrieve session data fails" when {
        ...
        "the fallback is successful and retrieves client MTDITID and NINO from the Session Cookie" should {
          ...
          "return session data" in {
            ...
            val result: UserSessionData = await(testService.getSessionData(sessionId)(request, hc))
            result shouldBe UserSessionData(sessionId = sessionId, mtditid = "12345678", nino = "AA111111A")
          }
        }
      }
