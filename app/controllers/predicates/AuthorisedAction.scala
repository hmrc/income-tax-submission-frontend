    ...
    authService.authorised.retrieve(allEnrolments and confidenceLevel) {
      case enrolments ~ userConfidence if userConfidence.level >= minimumConfidenceLevel =>
        ...
        (optionalMtdItId, optionalNino) match {
          case (Some(mtdItId), Some(nino)) => block(User(mtdItId, None, nino, hc.sessionId.getOrElse(SessionId(randomUUID.toString)).value))
          case (_, None) =>
        ...
        authService
          ...
          .retrieve(allEnrolments) { enrolments =>
            ...
            enrolmentGetIdentifierValue(EnrolmentKeys.Agent, EnrolmentIdentifiers.agentReference, enrolments) match {
              case Some(arn) =>
                block(User(mtdItId, Some(arn), nino, hc.sessionId.getOrElse(SessionId.apply(randomUUID.toString)).value))
              case None =>
