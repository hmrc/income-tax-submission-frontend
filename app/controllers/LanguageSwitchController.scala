...
class LanguageSwitchController @Inject()(appConfig: AppConfig,
  ...
  private def fallbackURL(implicit request: Request[_]): String = getSessionData[Int](SessionValues.TAX_YEAR) match {
    case Some(taxYear) => routes.StartPageController.show(taxYear).url
    case None => routes.StartPageController.show(appConfig.defaultTaxYear).url
  }
